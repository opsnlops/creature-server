 ...
 *
 * This is useful to help prevent data races on the clients. I don't want to
 * send a cache invalidate message _before_ the client is done thinking that
 * we're updating something on our side. This allows for a delay to occur,
 * leveraging our existing event loop.
 *
 * @param frameOffset how many frames from now should this invalidation be
 * scheduled?
 ...
 *
 * This is useful because the status contains the name of the currently running
 * animation, which can be helpful to show in the UI.
 *
