...
Database::getAllCreatures(creatures::SortBy sortBy, bool ascending,
    ...
    try {
        ...
        for (auto doc : cursor) {
            ...
            if (!result.isSuccess()) {
                ...
                std::string errorMessage =
                    fmt::format("Data format error while trying to get all of the creatures: {}", error.getMessage());
                critical(errorMessage);
                if (creatureSpan) {
        ...
        // Log the error
        std::string errorMessage =
            fmt::format("Data format error while trying to get all of the creatures: {}", e.what());
        error(errorMessage);
        if (dbSpan) {
        ...
        std::string errorMessage =
            fmt::format("A database error happened while getting all of the creatures: {}", e.what());
        error(errorMessage);
        if (dbSpan) {
            dbSpan->recordException(e);
            dbSpan->setAttribute("error.type", "DatabaseError");
