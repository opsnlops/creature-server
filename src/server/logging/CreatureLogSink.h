...
template <typename Mutex> class CreatureLogSink final : public base_sink<Mutex> {
  ...
  protected:
    ...
    static creatures::LogLevel mapSpdlogLevel(const level::level_enum spdlog_level) {
        switch (spdlog_level) {
        case level::trace:
            return creatures::LogLevel::trace;
        case level::debug:
            return creatures::LogLevel::debug;
        case level::info:
            return creatures::LogLevel::info;
        case level::warn:
            return creatures::LogLevel::warn;
        case level::err:
            return creatures::LogLevel::error;
        case level::critical:
            return creatures::LogLevel::critical;
        case level::off:
            return creatures::LogLevel::off;
        default:
            return creatures::LogLevel::unknown; // Default to unknown
        }
    }
