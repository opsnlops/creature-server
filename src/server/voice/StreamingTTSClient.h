...
/// One turn of a multi-character dialogue submission.
struct DialogueInput {
    std::string voiceId;
    std::string text; // may contain inline audio tags like [giggles]
};

/// One contiguous run of a single speaker in the mixed dialogue audio.
///
...
struct DialogueResult {
    ...
    std::vector<DialogueVoiceSegment> voiceSegments;
...
/// dialogue audio where the model's own timestamps are unreliable.
struct ForcedAlignmentResult {
    std::vector<ForcedAlignmentWord> words;
    std::vector<ForcedAlignmentChar> characters;
    /// Lower is better — empirically <0.1 on clean v3 dialogue audio.
    double loss = 0.0;
};
...
class StreamingTTSClient {
  public:
    ...
    /**
     * Generate multi-character dialogue via ElevenLabs Text-to-Dialogue API.
     *
     ...
     * @param parentSpan Optional observability span
     */
    Result<DialogueResult> generateDialogue(const std::string &apiKey, const std::vector<DialogueInput> &inputs,
     ...
     * rescue timing for eleven_v3 dialogue audio (whose own timestamps are broken).
     *
     * @param apiKey ElevenLabs API key
     * @param audio Audio bytes (WAV; same audio you got back from generateDialogue)
     ...
     *
     * Mirrors the dialogue prototype's strip_tags: removes "[...]" tags and runs
