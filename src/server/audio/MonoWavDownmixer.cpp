...
Result<std::shared_ptr<MonoWavStream>> MonoWavStream::open(const std::string &filePath, DownmixConfig downmixConfig) {
    ...
    const uintmax_t fileSize = std::filesystem::file_size(filePath, fileSizeError);
    const auto dataOffset = static_cast<std::streamoff>(dataPosition);
    if (fileSizeError || dataOffset < 0 || static_cast<uint64_t>(dataOffset) > fileSize ||
        static_cast<uint64_t>(dataSize) > fileSize - static_cast<uint64_t>(dataOffset)) {
        const auto errorMsg = fmt::format("WAV file '{}' declares audio data beyond the end of the file", filePath);
        return Result<std::shared_ptr<MonoWavStream>>{ServerError(ServerError::InvalidData, errorMsg)};
    }
