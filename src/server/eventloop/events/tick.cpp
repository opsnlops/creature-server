...
Result<framenum_t> TickEvent::executeImpl() {
    ...
    debug("⌚️ Hello from frame {:L}! Event queue length: {}, events: {:L}, frames streamed: {:L}, animations played: "
          ...
          "playlists events processed: {:L}, web api requests: {:L}",
          eventLoop->getCurrentFrameNumber(), eventLoop->getQueueSize(), metrics->getEventsProcessed(),
          metrics->getFramesStreamed(), metrics->getAnimationsPlayed(), metrics->getDMXEventsProcessed(),
          metrics->getSoundsPlayed(), metrics->getPlaylistsStarted(), metrics->getPlaylistsStopped(),
          metrics->getPlaylistsEventsProcessed(), metrics->getRestRequestsProcessed());
