    ...
    try {
        ...
        if (!collectionResult.isSuccess()) {
            ...
            std::string errorMessage =
                fmt::format("database error while listing all of the animations: {}", error.getMessage());
            warn(errorMessage);
        ...
        for (auto &&doc : cursor) {
            ...
            try {
                ...
                auto metadataSpan =
                    creatures::observability->createChildOperationSpan("Animation.metadataFromJson", docSpan);
                auto animationResult = animationMetadataFromJson(json_doc["metadata"]);

                if (!animationResult.isSuccess()) {
                    documentsFailed++;
                    std::string errorMessage = fmt::format("Unable to parse JSON to AnimationMetadata: {}",
                                                           animationResult.getError()->getMessage());
                    warn(errorMessage);

                    if (metadataSpan)
                        metadataSpan->setError(errorMessage);
                    if (docSpan)
