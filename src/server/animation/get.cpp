...
namespace creatures {
    ...
    Result<json> Database::getAnimationJson(animationId_t animationId, std::shared_ptr<OperationSpan> parentSpan) {
        ...
        try {
            ...
            if (!collectionResult.isSuccess()) {
                ...
                std::string errorMessage = fmt::format("Database error while attempting to get an animation by ID: {}",
                                                       error.getMessage());
                warn(errorMessage);
            ...
            if (maybe_result) {
                ...
                if (conversionSpan) {
                    ...
                    conversionSpan->setAttribute("json_string.size_bytes",
                                                 static_cast<int64_t>(bson_json_string.length()));
                }
                ...
                if (dbSpan) {
                    ...
                    if (json_result.contains("tracks") && json_result["tracks"].is_array()) {
                        dbSpan->setAttribute("animation.track_count",
                                             static_cast<int64_t>(json_result["tracks"].size()));
                    }
                }
            ...
            std::string errorMessage = fmt::format("a MongoDB error happened while loading an animation by ID: {}",
                                                   e.what());
    ...
    Result<creatures::Animation> Database::getAnimation(const animationId_t &animationId,
        ...
        if (dbSpan) {
            ...
            dbSpan->setAttribute("animation.duration_ms",
                                 static_cast<int64_t>(
                                     animation.metadata.number_of_frames * animation.metadata.milliseconds_per_frame));
