     ...
     */
     Result<creatures::Animation> Database::upsertAnimation(const std::string& animationJson, std::shared_ptr<OperationSpan> parentSpan) {
        ...
        try {
            ...
            if(parseSpan) parseSpan->setAttribute("input.size_bytes", static_cast<int64_t>(animationJson.length()));
            auto jsonObject = nlohmann::json::parse(animationJson);
            if(parseSpan) parseSpan->setSuccess();
            ...
            if (!animationResult.isSuccess()) {
                ...
                std::string errorMessage = fmt::format("Error while creating an animation from JSON: {}", error->getMessage());
                warn(errorMessage);
                if(validationSpan) validationSpan->setError(errorMessage);
                if(dbSpan) dbSpan->setError(errorMessage);
            ...
            if(!collectionResult.isSuccess()) {
                ...
                warn(errorMessage);
                if(collectionSpan) collectionSpan->setError(errorMessage);
                if(dbSpan) dbSpan->setError(errorMessage);
                return Result<creatures::Animation>{error};
            }
            auto collection = collectionResult.getValue().value();
            if(collectionSpan) collectionSpan->setSuccess();
            ...
            if(upsertSpan) upsertSpan->setSuccess();


            info("Animation upserted in the database: {}", animation.id);
            if(dbSpan) dbSpan->setSuccess();
            return Result<creatures::Animation>{animation};

        } catch (const mongocxx::exception &e) {
            std::string errorMessage = fmt::format("Error (mongocxx::exception) while upserting an animation in database: {}", e.what());
            error(errorMessage);
            if(dbSpan) {
            ...
            std::string errorMessage = fmt::format("Error (bsoncxx::exception) while upserting an animation in database: {}", e.what());
            error(errorMessage);
            if(dbSpan) {
                dbSpan->recordException(e);
                dbSpan->setError(errorMessage);
            ...
            std::string errorMessage = fmt::format("Error (nlohmann::json::exception) while upserting an animation in database: {}", e.what());
            error(errorMessage);
            if(dbSpan) {
                dbSpan->recordException(e);
                dbSpan->setError(errorMessage);
            ...
            if(dbSpan) dbSpan->setError(error_message);
