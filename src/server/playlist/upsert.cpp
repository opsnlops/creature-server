...
namespace creatures {
    ...
    Result<creatures::Playlist> Database::upsertPlaylist(const std::string& playlistJson, std::shared_ptr<OperationSpan> parentSpan) {
        ...
        try {
            ...
            if (!playlistResult.isSuccess()) {
                ...
                std::string errorMessage = fmt::format("Error while creating a playlist from JSON: {}", error->getMessage());
                warn(errorMessage);
