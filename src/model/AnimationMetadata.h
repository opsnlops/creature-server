...
#include OATPP_CODEGEN_BEGIN(DTO)
    ...
    class AnimationMetadataDto final : public oatpp::DTO {
        ...
        DTO_FIELD_INFO(title) {
            info->description = "The title for this animation";
        }
        DTO_FIELD(String, title);

        DTO_FIELD_INFO(milliseconds_per_frame) {
            info->description = "The number of milliseconds per frame (usually 20)";
        }
        DTO_FIELD(UInt32, milliseconds_per_frame);

        DTO_FIELD_INFO(note) {
            info->description = "Any notes to save in the database";
        }
        DTO_FIELD(String, note);

        DTO_FIELD_INFO(sound_file) {
            info->description = "The sound file to play with this animation";
        }
        DTO_FIELD(String, sound_file);

        DTO_FIELD_INFO(number_of_frames) {
            info->description = "The number of frames in the animation";
        }
        DTO_FIELD(UInt32, number_of_frames);

        DTO_FIELD_INFO(multitrack_audio) {
            info->description = "True if the audio is multitrack";
        }
