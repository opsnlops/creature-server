...
namespace creatures {
     ...
     * on the server's HAT. This is sent to the client so it can make a "virtual" set
     * of status lights if it wishes.
     */
...
class VirtualStatusLightsDto : public oatpp::DTO {
    ...
    DTO_FIELD_INFO(running) {
        info->description = "Is the event loop running?";
    }
    DTO_FIELD(Boolean, running);

    DTO_FIELD_INFO(dmx) {
        info->description = "Are we actively sending frames?";
    }
