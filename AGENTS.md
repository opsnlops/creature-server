...
- **`error.type` Honeycomb column is sticky-bool.** Some pre-3.11 write established the column as a boolean in Honeycomb's type inference; every string `error.type = "UnknownChannel"` etc. is now recorded as `false` in the UI. The const char* overload fix is working (other string columns record correctly). Fix needs Honeycomb-side action: delete the column in the dataset schema so it re-types on next write, or rename project-wide to a new attribute (40+ call sites — separate PR). Until then, error classifications aren't searchable by their string value.
