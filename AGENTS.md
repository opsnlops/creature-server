...
- miniaudio for in-process WAV/MP3/FLAC decoding
...
Native ALSA/CoreAudio playback is serialized through
`audio::LocalAudioPlaybackCoordinator` in local and travel modes.
`audio::NativeAudioPlaybackService` opens the exact configured device once,
keeps its queue warm with bounded silence, and owns it until shutdown. The
...
- Travel mode is the single-Raspberry-Pi-5 show rig and always renders stereo.
  Registered creature `audio_channel` lanes 1 and 2 route to output channels 1
  and 2; BGM lane 17 is mixed equally into both.
...
- The coordinator must be shut down and joined before the native playback
  service closes the ALSA/CoreAudio device.
...
- **macOS**: Uses system CoreAudio frameworks and resolv
- **Debian Linux**: Uses native ALSA, the UUID library via pkg-config, and OpenSSL 3.x (libssl3) for audio cache hashing
- **Raspberry Pi**: Specialized build scripts (`pi4_build.sh`); runs on Raspberry Pi OS (Debian-based)

## Deployment

Production installs the Debian package generated by CPack. GitHub Actions build,
test, and package natively on AMD64 and ARM64 runners. Docker is retained only as
a local macOS tool for building and testing Linux artifacts; it is not a runtime
