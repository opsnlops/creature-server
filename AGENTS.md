...
- **`error.type` and `http.flavor` Honeycomb columns are sticky-bool.** Pre-3.11 writes established these as booleans in Honeycomb's type inference; every string write (e.g. `error.type = "UnknownChannel"`, `http.flavor = "1.1"`) is now recorded as `false` in the UI. The const char* overload fix is working (other string columns like `error.channel_name` record correctly). Fix needs Honeycomb-side action: delete the columns in the dataset schema so they re-type on next write, or rename project-wide to new attribute keys. `error.type` has 40+ call sites (separate PR); `http.flavor` is one line in `ControllerUtils.h` and could be renamed to `http.protocol_version` (the modern OTel name) in a small follow-up. Until then, these classifications aren't searchable by their string value.
