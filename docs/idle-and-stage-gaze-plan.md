...
```
bearing(from P to T) = atan2(Tx - Px, Tz - Pz)     // degrees, CLOCKWISE from +Z
  0°   = facing the listener's side of the room (+Z)
 +90°  = facing +X (the listener's right)
 180°  = facing directly away from the listener

yaw runs the OTHER way round the circle — COUNTERCLOCKWISE from +Z:
  yaw = 0   → bird faces +Z
  yaw = +90 → bird faces -X (the listener's LEFT)

relative_angle = normalize180(bearing_to_target - facingBearingDegrees(yaw))
where facingBearingDegrees(yaw) = normalize180(-yaw)
```

> **Corrected 2026-08-16 — issue [#144](https://github.com/opsnlops/creature-server/issues/144).**
> This section originally claimed yaw used the *same* convention as `bearing`, and the
> implementation followed it (`relative = bearing - yaw`). It is wrong: the Console writes
> yaw counterclockwise. Subtracting a counterclockwise angle from a clockwise one doubles
> the error instead of cancelling it, so on the real Mainstage every creature-to-creature
> angle came out at 78–164° — past every neck — and all 13 rendered animations pinned
> their necks to a mechanical rail for the entire scene. Every yaw figure in the tables
> below is stated in the *old* sign; negate them to read the stage documents as they are
> actually stored. The geometry itself (the conversation V, birds angled inward) is
> unchanged.
...
### What the first real renders showed (issue #144)

Rendering 13 dialogs against the real Mainstage exposed three things the paper
model got wrong. Recorded here because two of them are still open.

**1. The yaw sign was inverted — fixed.** See the correction above. Effect on
Beaky's pan spread across all her targets: 74 bytes before, 173 after. Caroll
resolved all six targets into 26 bytes; she now gets 189.

**2. Elevation carries no target information on the real stage — still open.**
The design's headline claim is *"pan says which side, elevation says which
one."* That needs differing perch heights. On Mainstage **five of six creatures
sit at `y = 0`**; only Mango differs. So Beaky's elevation byte is 128 for every
target except Mango, and the disambiguation channel the design leans on simply
does not exist. The plan already flagged measuring perch heights as "the single
highest-value input" — it is now the blocking one.

**3. Gaze overwrites authored head motion with a dead constant — still open.**
Layer 2 overrides Layer 0 absolutely, so wherever gaze drives an axis the loop's
motion on that axis is discarded. Kenny is an accidental control group (he has
no `gaze` block). In *UU - You might be UU if — Mainstage*:

| creature | neck_rotate | head_tilt | head_height |
|---|---|---|---|
| Kenny (no gaze) | 156 distinct (loop: 156) | 159 (loop: 159) | 125 (loop: 125) |
| Beaky (gaze) | 14 (loop: 99) | **1** (loop: 82) | 32 (loop: 46) |

Beaky's `head_tilt` is the literal constant 128 for every frame in 6 of the 13
renders, because the cock axis centres whenever a creature is speaking. On a
differential neck that removes most of the neck's liveliness. The decision is
that gaze wins on its axes — so the fix is for the cock to *do something* while
speaking rather than to blend, but that is not yet designed.

**Not a defect: `stand_rotate`.** The motion visible on that slot is passed
through from the authored loops ("Beaky Speech Loop 1" drives it 85–186), not
generated by the gaze layer, which only ever writes the pan/elevation/cock
slots. It became visible because idle-during-silence replaced the old freeze.
`stand_rotate` is not a connected axis, so the loops should stop driving it.
