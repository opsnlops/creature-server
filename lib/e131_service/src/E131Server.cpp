...
void E131Server::init(uint16_t _networkDevice, std::string _version) {
    ...
    // Ensure null termination in case gethostname filled the entire buffer
    hostname[sizeof(hostname) - 1] = '\0';
    ...
    // Ensure we don't exceed SOURCE_NAME_LENGTH and always null-terminate
    size_t copySize = std::min(sourceHeader.size(), static_cast<size_t>(SOURCE_NAME_LENGTH - 1));
    memcpy(this->sourceName, sourceHeader.c_str(), copySize);
    this->sourceName[copySize] = '\0';
    ...
    socket = e131_socket();
    if (socket < 0) {
        logger->critical("e131_socket failed: {}", strerror(errno));
        throw std::runtime_error("Failed to create E1.31 socket");
    }

    // Join our multicast group
    if (e131_multicast_iface(socket, this->networkDevice) < 0) {
        logger->critical("e131_multicast_iface failed: {}", strerror(errno));
        close(socket);
        socket = -1;
...
void E131Server::setValues(uint16_t universeNumber, uint16_t firstSlot, std::vector<uint8_t> &values) {
    // Validate universe number to prevent DoS attacks with excessive memory usage
    if (universeNumber == 0 || universeNumber > 63999) {
        logger->error("Invalid universe number: {}", universeNumber);
        return;
    }

    // Validate slot and data size
    if (firstSlot == 0 || firstSlot > 512) {
        logger->error("Invalid first slot: {}", firstSlot);
        return;
    }

    if (values.size() > 512) {
        logger->error("Data size too large: {}", values.size());
        return;
    }
